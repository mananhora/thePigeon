<?php
   ...
   $sqlletterid = "SELECT letterid FROM letter WHERE letterid IN (SELECT letterid FROM mailbox WHERE receiveruserid = (SELECT userid FROM user WHERE userid = ".$useridobtained.")) ORDER BY letterid DESC LIMIT 1 OFFSET 1;";
   ...
   if($letterid == ''){
      $sqlletterid = "SELECT letterid FROM letter WHERE letterid IN (SELECT letterid FROM mailbox WHERE receiveruserid = (SELECT userid FROM user WHERE userid = 2)) ORDER BY letterid DESC LIMIT 1 OFFSET 1;";
      $ret = $db->query($sqlletterid);
      while($row = $ret->fetchArray(SQLITE3_ASSOC) ){
        $letterid = $row['letterid'];
      }
