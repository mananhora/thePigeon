<?php
   ...
   CREATE TABLE friendrequest (friendrequestid integer primary key autoincrement, timestamp default current_timestamp not null, senderuserid integer not null, receiveruserid integer not null, foreign key(senderuserid) REFERENCES user(userid), foreign key(receiveruserid) REFERENCES user(userid));
   ";
   $ret = $db->exec($sql);
   if(!$ret){
      echo $db->lastErrorMsg();
