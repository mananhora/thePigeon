<?php
   ...
   // Return letters received by this user, sorted from most recent to least recent, skipping 8 results.
   $sql = "SELECT content FROM letter WHERE letterid IN (SELECT letterid FROM mailbox WHERE receiveruserid = (SELECT userid FROM user WHERE userid = ".$useridobtained.")) ORDER BY letterid DESC LIMIT 1 OFFSET 8;";
   $ret = $db->query($sql);
   while($row = $ret->fetchArray(SQLITE3_ASSOC) ){
     echo $row['content'];
   }
